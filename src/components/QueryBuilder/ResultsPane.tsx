...
export function ResultsPane({
  ...
  const SortIcon = ({ field }: { field: string }) => {
    ...
    if (sortConfig.direction === 'asc') return <ArrowUp size={12} className="text-orange-500" />;
    return <ArrowDown size={12} className="text-orange-500" />;
  };
      ...
      {!hasExecuted && !isLoading && (
        <div className="surface-base rounded-xl p-12 flex flex-col items-center justify-center gap-3 text-center">
          <div className="p-4 bg-orange-50 dark:bg-orange-500/10 rounded-2xl">
            <Play size={32} className="text-orange-500 dark:text-orange-400" />
          </div>
          <h3 className="heading text-[18px] text-slate-700 dark:text-zinc-300">
            Ready to execute
          </h3>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-2 text-slate-500 dark:text-zinc-400">
              ...
              <select
                ...
                className="px-2 py-1 bg-white dark:bg-white/10 border border-zinc-200 dark:border-white/10 rounded-lg text-sm outline-none focus:ring-2 focus:ring-orange-500/20"
              >
                {PAGE_SIZES.map((s) => (
